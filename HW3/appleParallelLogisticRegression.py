...
def mapFeaturesToPartitionsRDD(groupedDataRDD,N):
    ...
    """
    return groupedDataRDD.flatMapValues(lambda datalist: list(getAllFeatures(datalist))) \
...
def sendToPartitions(betaRDD, featuresToPartitionsRDD, N):
    ...
    #     .reduceByKey(lambda x, y: x + y).partitionBy(N, identityHash).cache()

    # return featuresToPartitionsRDD.join(betaRDD) \
    #         .map(lambda pair: (pair[1][0], [pair[0],pair[1][1]])) \
    #         .reduceByKey(lambda x,y: x+y).values().map(SparseVector)

    return featuresToPartitionsRDD.join(betaRDD).map(lambda pair: (pair[1][0], [pair[0], pair[1][1]])) \
            .groupByKey().mapValues(SparseVector).partitionBy(N, identityHash).cache()
...
def totalLossRDD(groupedDataRDD,featuresToPartitionsRDD,betaRDD,N,lam = 0.0):
    ...
    # loss = groupedDataRDD.join(beta_small).values() \
    #     .map(lambda tupl: totalLoss(tupl[0],tupl[1],lam=0.0)) \
    #     .reduce(lambda x,y: x+y)

    beta_small = sendToPartitions(betaRDD, featuresToPartitionsRDD, N)
    loss = groupedDataRDD.join(beta_small) \
        .mapValues(lambda tupl: totalLoss(tupl[0],tupl[1],lam=0.0)) \
        .values().sum()

    if lam != 0.0:
        loss = loss + lam*betaRDD.values().map(lambda x: x**2).sum()
...
def gradTotalLossRDD(groupedDataRDD,featuresToPartitionsRDD,betaRDD,N,lam = 0.0):
    ...
    beta_small = sendToPartitions(betaRDD, featuresToPartitionsRDD, N)
    # grad = groupedDataRDD.join(beta_small).values() \
    #     .flatMap(lambda tupl: list( gradTotalLoss(tupl[0], tupl[1],lam=0.0).items()) )\
    #     .reduceByKey(lambda x, y: x + y)
    grad = groupedDataRDD.join(beta_small) \
            .flatMapValues(lambda tupl: list( gradTotalLoss(tupl[0], tupl[1],lam=0.0).items())) \
            .values().reduceByKey(lambda x,y: x+y)

    if lam != 0.0:
        norm_grad = betaRDD.mapValues(lambda x: 2*x*lam)

        grad = grad.join(norm_grad).mapValues(lambda x: np.sum(x))
...
def basicMetricsRDD(groupedDataRDD,featuresToPartitionsRDD,betaRDD,N):
    ...
    """
    beta_small = sendToPartitions(betaRDD, featuresToPartitionsRDD, N)
    metrics_ = groupedDataRDD.join(beta_small).values() \
        .map(lambda tupl: basicMetrics(tupl[0], tupl[1])) \
        .reduce(lambda x,y: np.add(x,y))

    return metrics_.tolist()
