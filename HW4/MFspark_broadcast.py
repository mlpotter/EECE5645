...
def readRatings(f, sparkContext):
    ...
    """
    return sparkContext.textFile(f).map(lambda x: tuple(x.split(','))).map(
        lambda inp: (int(inp[0]), int(inp[1]), float(inp[2])))
...
def joinAndPredictAll(R, U, V, N):
    ...
    ijduv = jiruv.map(lambda tuple: (
    tuple[1][0][0], tuple[0], pred_diff(tuple[1][0][1], tuple[1][0][2], tuple[1][1]), tuple[1][0][2],
    tuple[1][1]))  # create tuples of (i,j,d,ui,vj)
...
def adaptU(joinedRDD, gamma, lam, N):
    ...
    grad_u = joinedRDD.map(lambda tuple: (tuple[0], gradient_u(tuple[2], tuple[3], tuple[4]))).reduceByKey(
        lambda grad1, grad2: grad1 + grad2, numPartitions=N)
...
def adaptV(joinedRDD, gamma, mu, N):
    ...
    """
    # get the summation of the gradients for the first part of RegSE
    grad_v = joinedRDD.map(lambda tuple: (tuple[1], gradient_v(tuple[2], tuple[3], tuple[4]))).reduceByKey(
        lambda grad1, grad2: grad1 + grad2, numPartitions=N)
...
if __name__ == "__main__":
    ...
    # SparkContext.setSystemProperty("spark.task.cpus",15)
