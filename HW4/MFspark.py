...
def joinAndPredictAll(R,U,V,N):
    ...
    ijru = R.map(lambda tuple: (tuple[0], (tuple[1], tuple[2]))).join(U, numPartitions=N)  # create tuples of (i,((j,rij),ui))

    jiru = ijru.map(lambda tuple: (tuple[1][0][0], (tuple[0], tuple[1][0][1], tuple[1][1])))  # create tuples of (j,(i,rij,ui))

    jiruv = jiru.join(V, numPartitions=N)  # create tuples of (j,((i,rij,ui),vj)))

    ijduv = jiruv.map(lambda tuple: (tuple[1][0][0],tuple[0], pred_diff(tuple[1][0][1], tuple[1][0][2], tuple[1][1]), tuple[1][0][2],tuple[1][1]))  # create tuples of (i,j,d,ui,vj)

    # ijuv = U.cartesian(V).map(lambda tuple: ((tuple[0][0], tuple[1][0]), (tuple[0][1], tuple[1][1])))
    # ijr = R.map(lambda tuple: ((tuple[0], tuple[1]), tuple[2]))
    # ijduv = ijr.join(ijuv, numPartitions=N).map(lambda tuple: (tuple[0][0], tuple[0][1], pred_diff(tuple[1][0], tuple[1][1][0], tuple[1][1][1]), tuple[1][1][0], tuple[1][1][1]))
...
def SE(joinedRDD):
    ...
    """
    return joinedRDD.map(lambda tuple: tuple[2]**2).sum()
...
def normSqRDD(profileRDD,param):
	...
	The return value is:
        λ Σ_i ||ui||_2^2 	
    """
    return param*profileRDD.map(lambda tuple: tuple[1]@tuple[1]).sum()
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    """
    # get the summation of the gradients for the first part of RegSE
    grad_u = joinedRDD.map(lambda tuple: (tuple[0], gradient_u(tuple[2], tuple[3], tuple[4]))).reduceByKey(lambda grad1, grad2: grad1 + grad2,numPartitions=N)

    # get the unique elements (i,ui)
    iui = joinedRDD.map(lambda tuple: (tuple[0], tuple[3])).reduceByKey(lambda ui, empty: ui,numPartitions=N)

    # join the unique elements with their gradients AND add in the lam term
    return iui.join(grad_u, numPartitions=N).mapValues(lambda tuple: tuple[0] - gamma * (tuple[1] + 2 * lam * tuple[0]))
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    """
    # get the summation of the gradients for the first part of RegSE
    grad_v = joinedRDD.map(lambda tuple: (tuple[1], gradient_v(tuple[2], tuple[3], tuple[4]))).reduceByKey(lambda grad1, grad2: grad1 + grad2,numPartitions=N)

    # get the unique elements (j,vj)
    jvj = joinedRDD.map(lambda tuple: (tuple[1], tuple[4])).reduceByKey(lambda vj, empty: vj,numPartitions=N)

    # join the unique elements with their gradients AND add in the lam term
    return jvj.join(grad_v, numPartitions=N).mapValues(lambda tuple: tuple[0] - gamma * (tuple[1] + 2 * mu * tuple[0]))
