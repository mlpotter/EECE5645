...
def readRatings(file, sparkContext):
    ...
    return sparkContext.textFile(file).map(lambda x: tuple(x.split(','))).map(
        lambda tuple: (int(tuple[0]), int(tuple[1]), float(tuple[2])))
...
def generateUserProfiles(R, d, seed, sparkContext, N):
    ...
    """
    # extract user ids
    U = R.map(lambda tuple: tuple[0]).distinct(numPartitions=N)
    numUsers = U.count()
...
def generateItemProfiles(R, d, seed, sparkContext, N):
    ...
    """
    # extract item ids
    V = R.map(lambda tuple: tuple[1]).distinct(numPartitions=N)
    numItems = V.count()
...
def joinAndPredictAll(R, U, V, N):
    ...
    """
    # make keys common keys i
    R_i_jr = R.map(lambda tuple: (tuple[0], (tuple[1], tuple[2])))
    # join and make common keys j
    j_iur = R_i_jr.join(U, numPartitions=N).map(lambda tuple: (tuple[1][0][0], (tuple[0], tuple[1][1], tuple[1][0][1])))
    # join all and apply pred_diff(rij, ui, vj)
    return j_iur.join(V, numPartitions=N).map(lambda tuple: (tuple[1][0][0], tuple[0], pred_diff(tuple[1][0][2], tuple[1][0][1], tuple[1][1]), tuple[1][0][1], tuple[1][1]))
...
def SE(joinedRDD):
    ...
    """
    return joinedRDD.map(lambda tuple: tuple[2] ** 2).reduce(lambda del1, del2: del1 + del2)
...
def normSqRDD(profileRDD, param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        λ Σ_i ||ui||_2^2
    """
    return param * profileRDD.map(lambda tuple: np.linalg.norm(tuple[1]) ** 2).reduce(lambda u1, u2: u1 + u2)
...
def adaptU(joinedRDD, gamma, lam, N):
    ...
    """
    # calculate gradSE for each (i,ui)
    grad_u_SE = joinedRDD.map(lambda tuple: (tuple[0], gradient_u(tuple[2], tuple[3], tuple[4]))). \
        reduceByKey(lambda grad1, grad2: grad1 + grad2, N)
    # join with previous ui
    i_ui_gradSE = joinedRDD.map(lambda tuple: (tuple[0], tuple[3])).reduceByKey(lambda ui, temp: ui, N).join(
        grad_u_SE, N)
    # add regularization
    i_ui_gradRSE = i_ui_gradSE.mapValues(lambda tuple: (tuple[0], tuple[1] + 2 * lam * tuple[0]))
    # update ui
    return i_ui_gradRSE.map(lambda tuple: (tuple[0], tuple[1][0] - gamma * tuple[1][1]))
...
def adaptV(joinedRDD, gamma, mu, N):
    ...
    """
    # calculate gradSE for each (j,vj)
    grad_v_SE = joinedRDD.map(lambda tuple: (tuple[1], gradient_v(tuple[2], tuple[3], tuple[3]))). \
        reduceByKey(lambda grad1, grad2: grad1 + grad2, N)
    # join with previous vj
    j_vj_gradSE = joinedRDD.map(lambda tuple: (tuple[1], tuple[4])).reduceByKey(lambda vj, temp: vj, N).join(
        grad_v_SE, N)
    # add regularization
    j_vj_gradRSE = j_vj_gradSE.mapValues(lambda tuple: (tuple[0], tuple[1] + 2 * mu * tuple[0]))
    # update ui
    return j_vj_gradRSE.map(lambda tuple: (tuple[0], tuple[1][0] - gamma * tuple[1][1]))



if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Parallel Matrix Factorization.',
    ...
    for k in folds:
        ...
        print
        "Training set contains %d users and %d items" % (U.count(), V.count())
